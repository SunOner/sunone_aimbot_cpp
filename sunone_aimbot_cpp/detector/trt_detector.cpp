...
std::vector<std::vector<Detection>> TrtDetector::detectBatch(const std::vector<cv::Mat>& frames)
{
    ...
    // --- FIN DE LA OPTIMIZACI�N ---

    // El resto del c�digo permanece igual, ya que es espec�fico de la inferencia y el postprocesado.
    context->enqueueV3(stream);
    cudaStreamSynchronize(stream);

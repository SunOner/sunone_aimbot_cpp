    ...
    , right_button_(false)
{
    try
    {
        ...
        if (serial_.isOpen()) {
            ...
            if (initializeButtonReporting()) { // Se ejecutará al baud_rate
...
std::string Kmbox_b_Connection::read() {
    if (!is_open_.load()) return std::string();
    std::string result;
    try {
...
bool Kmbox_b_Connection::initializeButtonReporting() {
    ...
    try { if (serial_.isOpen()) current_br = serial_.getBaudrate(); }
    catch (...) {}
    ...
    try {
        if (serial_.available() > 0) {
            ...
            for (char c : flushed_data) {
                if (isprint(c)) oss_flush << c; else oss_flush << "[" << std::hex << std::setw(2) << std::setfill('0') << (0xFF & static_cast<unsigned char>(c)) << "]";
            }
...
void Kmbox_b_Connection::press(int button) {
    std::string cmd;
    if (button == 0) cmd = "km.left(1)";
    else if (button == 1) cmd = "km.right(1)";
    else if (button == 2) cmd = "km.middle(1)";
    else { std::cerr << "[Kmbox_b] Press: Unknown button_id: " << button << std::endl; return; }
    sendCommand(cmd);
}

void Kmbox_b_Connection::release(int button) {
    std::string cmd;
    if (button == 0) cmd = "km.left(0)";
    else if (button == 1) cmd = "km.right(0)";
    else if (button == 2) cmd = "km.middle(0)";
    else { std::cerr << "[Kmbox_b] Release: Unknown button_id: " << button << std::endl; return; }
    sendCommand(cmd);
}
...
void Kmbox_b_Connection::listeningThreadFunc() {
    ...
    try { if (serial_.isOpen()) current_br_listener = serial_.getBaudrate(); }
    catch (...) {}
    ...
    while (listening_.load() && is_open_.load()) {
        ...
        try {
            if (!serial_.isOpen()) {
                if (loop_counter % no_data_log_interval == 1) std::cout << "[Kmbox_b] BIN Listener: Port seems closed." << std::endl;
                is_open_ = false;
                break;
            }
...
int Kmbox_b_Connection::monitorMouseLeft() const {
    if (!isOpen()) return -1; // Opcional: chequear si está abierto
